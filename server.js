...
// Auto-copy the APK to the root workspace directory
const srcApk = path.join(__dirname, 'app', 'build', 'outputs', 'apk', 'debug', 'app-debug.apk');
const destApk = path.join(__dirname, 'Naam_Jap_Reminder_v1.0.0.apk');
try {
    if (fs.existsSync(srcApk)) {
        fs.copyFileSync(srcApk, destApk);
        console.log(`[APK COPY] Successfully copied APK to root: ${destApk}`);
    } else {
        console.log('[APK COPY] Source APK not found at: ' + srcApk);
    }
} catch (err) {
    console.error('[APK COPY] Failed to copy APK:', err);
}
...
// Robust APK serving function that checks both possible paths
const serveApk = (req, res) => {
    const possiblePaths = [
        path.join(__dirname, 'Naam_Jap_Reminder_v1.0.0.apk'),
...
// Download compiled APK endpoints (support multiple URLs for maximum compatibility)
app.get('/download/app-debug.apk', serveApk);
app.get('/download/app-debug.apk/', serveApk);
app.get('/download/Naam_Jap_Reminder_v1.0.0.apk', serveApk);
app.get('/download/Naam_Jap_Reminder_v1.0.0.apk/', serveApk);
app.get('/Naam_Jap_Reminder_v1.0.0.apk', serveApk);
app.get('/Naam_Jap_Reminder_v1.0.0.apk/', serveApk);
